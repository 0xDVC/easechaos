...
export default function DayView({ schedule }: DayViewProps) {
  ...
  processedEvents.forEach((event, index) => {
    const overlappingEvents = processedEvents.filter(
      (otherEvent, otherIndex) => {
        if (otherIndex === index) return false;
        const sameTimeSlot =
          convertTimeToNumber(otherEvent.start) ===
            convertTimeToNumber(event.start) &&
    ...
    if (overlappingEvents.length > 0) {
      ...
      event.splitIndex =
        overlappingEvents.findIndex((e) => e.value > event.value) + 1;
    }
  });

  const mergedEvents = processedEvents.reduce(
    (acc: PositionedEvent[], event) => {
      ...
      if (previousEvent) {
        ...
        previousEvent.duration =
          ((convertTimeToNumber(event.end) -
            convertTimeToNumber(previousEvent.start)) /
            13) *
          100;
      } else {
        acc.push(event);
            ...
            {timeSlots.map((hour, index) => (
              <div
                ...
                style={{
                  top: `${(index / (timeSlots.length - 1)) * 100}%`,
                  height: `${
                    index % 2 === 0 ? 100 / (timeSlots.length - 1) : 0
                  }%`,
